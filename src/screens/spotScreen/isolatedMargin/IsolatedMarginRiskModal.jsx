...
const IsolatedMarginRiskModal = forwardRef(({ row }, ref) => {
  ...
  const fetchLive = useCallback(() => {
    ...
    appOperation.get(`margin/account/${pairId}`, undefined, undefined, CUSTOMER_TYPE)
      .then((res) => { if (res?.success) setLiveAccount(res.data); })
      .catch(() => {});
  }, [pairId]);
  ...
  const account = useMemo(() => ({
    ...(row || {}),
    ...(liveAccount || {}),
    margin_call_level: liveAccount?.margin_call_level ?? row?.margin_call_level ?? row?.marginCallLevel,
    liquidation_margin_level: liveAccount?.liquidation_margin_level ?? row?.liquidation_margin_level ?? row?.liquidationMarginLevel,
    leverage: liveAccount?.leverage ?? liveAccount?.default_leverage ?? row?.leverage ?? row?.default_leverage,
    effective_multiple: liveAccount?.effective_multiple ?? liveAccount?.effective_leverage ?? row?.effective_multiple ?? row?.effective_leverage ?? row?.effectiveMultiple,
    margin_level: liveAccount?.margin_level ?? row?.margin_level ?? row?.marginLevel,
  }), [row, liveAccount]);

  const leverage = parseMarginLevel(account.leverage ?? account.default_leverage ?? account.max_leverage);
  const effectiveMultiple = parseEffectiveMultiple(account);
  const thresholds = resolveMarginThresholds(account);
  const hasDebt = pairHasDebt(account);
  const ml = hasDebt ? parseMarginLevel(account.margin_level) : null;
  const status = getMarginLevelStatus(ml, thresholds, { hasDebt });
  const initialMl = computeInitialMarginLevel(effectiveMultiple ?? leverage);
  const callFmt = formatThresholdLevel(thresholds.marginCallLevel);
  const liqFmt = formatThresholdLevel(thresholds.liquidationMarginLevel);
  const thresholdsReady = thresholds.marginCallLevel != null && thresholds.liquidationMarginLevel != null;
  const pairDisplay = String(account.pairRaw || account.pair || "—").replace("/", "");
  const effectiveMultipleLabel = effectiveMultiple ? `${effectiveMultiple}x` : null;
        ...
        <View style={styles.header}>
          <View style={{ flex: 1, paddingRight: 12 }}>
            ...
            <AppText style={{ fontSize: 13, lineHeight: 18, color: palette.muted }}>
              {`Isolated position, ${pairDisplay}${effectiveMultipleLabel ? ` · Effective multiple ${effectiveMultipleLabel}` : ""}`}
            </AppText>
          </View>
        ...
        >
          <View style={{ marginBottom: 18 }}>
            ...
            <AppText style={{ fontSize: 12, lineHeight: 17, color: palette.muted }}>
              Margin level = asset value ÷ (debt + unpaid interest), at mark price
            </AppText>
          </View>
          ...
          {thresholdsReady ? (
            <>
              <RiskMeter
                ml={ml}
                liqLevel={thresholds.liquidationMarginLevel}
                callLevel={thresholds.marginCallLevel}
                hasDebt={hasDebt}
                palette={palette}
              />
              <View style={{ gap: 4, marginBottom: 16 }}>
                <RiskTierRow
                  ...
                  subtitle="Borrowing and transfers out are allowed if the level stays above the margin call level afterward."
                ...
                <RiskTierRow
                  ...
                  subtitle="Add collateral or repay debt. New borrowing and transfers out are blocked; trading still works."
                ...
                <RiskTierRow
                  ...
                  subtitle="Open orders are cancelled and the position closes at market. A 2% liquidation fee applies."
          ...
          {initialMl != null && (effectiveMultiple || leverage) ? (
            <AppText style={[styles.footnote, { color: palette.muted, borderTopColor: palette.border }]}>
              {`At full ${effectiveMultipleLabel || `${leverage}x`} with no extra collateral, the starting margin level is ${formatMarginLevel(initialMl)}.`}
            </AppText>
          ) : null}
          ...
          <View style={{ borderTopWidth: 1, borderTopColor: palette.border }}>
            ...
            >
              <AppText style={{ fontSize: 12, lineHeight: 17, color: palette.muted, marginBottom: 10 }}>
                Uses the same margin call level as the risk bands above.
              </AppText>
              {thresholdsReady ? (
                <View style={{ gap: 8 }}>
                  <StatusRule allowed label="Borrowable" value={`ML > ${callFmt}`} palette={palette} />
                  <StatusRule label="Not borrowable" value={`ML ≤ ${callFmt}`} palette={palette} />
                </View>
              ) : null}
            </CollapsibleSection>
            ...
            >
              <AppText style={{ fontSize: 12, lineHeight: 17, color: palette.muted, marginBottom: 10 }}>
                Transfer-out requires ML above the margin call level before and after the transfer.
              </AppText>
              {thresholdsReady ? (
                <View style={{ gap: 8 }}>
                  <StatusRule allowed label="Permitted" value={`ML > ${callFmt}`} palette={palette} />
                  <StatusRule label="Not permitted" value={`ML ≤ ${callFmt}`} palette={palette} />
                </View>
              ) : null}
            </CollapsibleSection>
          </View>
...
IsolatedMarginRiskModal.displayName = "IsolatedMarginRiskModal";

export default IsolatedMarginRiskModal;
